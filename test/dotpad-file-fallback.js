...
(async () => {
  // ── 1. 내장 SDK: 파일 선택 없이 즉시 로드 (공식 SDK가 실제로 import되는지) ──
  {
    ...
    await page.evaluate(() => { DOTPAD.BLE.MIN_INTERVAL = 5; DOTPAD.BLE.HEAVY_GAP = 20; });
  ...
  for (const [sdkPath, label] of [[ESM_SDK, 'ESM picker'], [UMD_SDK, 'UMD picker']]) {
    ...
    await page.goto(DIST);
    await page.waitForSelector('#gallery .gcat');
    await page.evaluate(() => { DOTPAD.BLE.MIN_INTERVAL = 5; DOTPAD.BLE.HEAVY_GAP = 20; });   // 테스트 가속 (전송 간격은 dotpad-pacing에서 검증)
