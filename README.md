...
공식 `DotPadSDK-3.0.0.js`가 필요하다 (라이선스상 HTML에 내장 불가).

- 파일을 디스크에서 직접(file://) 연 경우: "DotPad 연결" 클릭 → 파일 선택 창에서 SDK 파일 선택 (브라우저 CORS 정책 때문에 한 번만 수동 선택).
- 웹서버로 서빙하는 경우: HTML과 같은 폴더에 SDK를 두면 자동 로드.
